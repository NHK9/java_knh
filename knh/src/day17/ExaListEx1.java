...
public class ExaListEx1 {
	...
	public static void program(Scanner scan,ArrayList<ExbStudent> stdList) {
		...
		for(;;) {
			...
			}else System.out.println("일치하는 학생정보가 없습니다");
			break;
	...
	/* 기능: Scanner를 이용해 수정할 학생 정보와 성적, 이름을 입력받고,
	 * 		입력받은 학생 정보를 주어진 리스트에서 수정하여 수정됐는지 안됐는지
	 * 		알려주는 메소드
	...
	public static boolean modifyStudent(Scanner scan, ArrayList<ExbStudent> stdList) {
		...
		if(index >=0) {
			...
			// 참조변수는 주소를 공유하므로 std에서 수정한 값이 stdList로 들어가는 방법 이용
