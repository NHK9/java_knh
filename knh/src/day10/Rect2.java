...
public class Rect2 {
	//양 끝 점을 이용한 사각형(그림판기준 : 윗점 < 아래점)
	private int left; //왼쪽위 점의 x좌표
	private int top;  //왼쪽위 점의 y좌표
	private  int right; //오른쪽 아래점의 x좌표
	private  int bottom;//오른쪽 아래점의 y좌표
	...
	public void move (int x, int y) {
		...
		left = x; top = y;
		right = x+width; bottom = y + heigth;
	}
	//사각형 크기변경 기능(왼쪽 위의점 기준으로 크기변경)
	public void resize(int width, int heigth) {
		right = left+width; bottom = top+heigth;
	}
}
